...
public class CheckBoxIcon implements Icon, UIResource {
	...
	public function updateIcon(c : Component, g : Graphics2D, x : int, y : int) : void {
		...
		if (drawDot) {
			...
			g.lineTo(x + w + 2.5, y);
			g.endDraw();
			dot.filters = [new DropShadowFilter(0, 45, mcls.bdark.getRGB(), mcls.shadow, 2, 2, 4, 1, false)];
		}
		dot.visible = drawDot;
		box.alpha = alpha;
		box.filters = [new DropShadowFilter(innerDis, 45, 0x0, style.shadow * shadowScale, 5, 5, 1, 1, true), new BevelFilter(1, 45, 0x0, style.shadow, 0xFFFFFF, style.shadow, 3, 3, 1, 1, BitmapFilterType.FULL)];
	}
