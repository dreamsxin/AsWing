...
public class RadioButtonIcon implements Icon, UIResource {
	...
	public function updateIcon(c : Component, g : Graphics2D, x : int, y : int) : void {
		...
		if (drawDot) {
			...
			g.fillCircle(new GradientBrush(GradientBrush.RADIAL, [mcls.clight.getRGB(), mcls.cdark.getRGB()], [mcls.clight.getAlpha(), mcls.cdark.getAlpha()], [0, 200], matrix), x + w / 2, y + h / 2, r);
		}
		shape.alpha = alpha;
		shape.filters = [new DropShadowFilter(innerDis, 45, 0x0, style.shadow * shadowScale, 5, 5, 1, 1, true), new BevelFilter(1, 45, 0x0, style.shadow, 0xFFFFFF, style.shadow, 3, 3, 1, 1, BitmapFilterType.FULL)];
	}
